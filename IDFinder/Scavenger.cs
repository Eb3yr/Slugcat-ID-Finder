...
namespace IDFinder
{
    public class Scavenger
    {
        ...
        public Scavenger(int ID, bool isElite = false)
        {
            ...
            float num2 = float.Lerp(0.5f, 1.5f, float.Pow(XORShift128.Shared.NextFloat(), 1.5f - Personality.Aggression));
            num2 = float.Lerp(num2, num2 * Custom.LerpMap(teeth.GetLength(0), 4f, 8f, 1f, 0.5f), 0.3f);
            float num3 = float.Lerp(num2 + 0.2f, float.Lerp(0.7f, 1.2f, XORShift128.Shared.NextFloat()), XORShift128.Shared.NextFloat());
    ...
    public struct Eartlers
    {
		...
		private void GenerateSegments(bool elite)
        {
            ...
            list.Add(new(Custom.DegToFloat2(float.Lerp(40f, 90f, XORShift128.Shared.NextFloat())) * 0.4f * rhs, 1f * num2));
            Vector2 float2 = Custom.DegToFloat2(float.Lerp(@float.X, @float.Y, XORShift128.Shared.NextFloat()) * num);
            Vector2 float3 = float2 - Custom.DegToFloat2(float.Lerp(40f, 90f, XORShift128.Shared.NextFloat()) * 0.4f * rhs);
            if (float3.X < 0.2f)
            ...
            Vector2 float4 = Vector2.Lerp(Points[0][1].pos, Points[0][2].pos, float.Lerp(0f, num7 == 1 ? 0.7f : 0.25f, XORShift128.Shared.NextFloat()));
            ...
            {
                list.Clear();
                float4 = Vector2.Lerp(Points[0][1].pos, Points[0][2].pos, float.Lerp(0.45f, 0.7f, XORShift128.Shared.NextFloat()));
                list.Add(new(float4, 1.2f));
                list.Add(new(float4 + Points[0][3].pos - Points[0][2].pos + Custom.DegToFloat2(XORShift128.Shared.NextFloat() * 360f) * 0.1f, 1.75f));
                DefineBranch(list);
            }
            ...
            {
                list.Clear();
                Vector2 float5 = Custom.DegToFloat2(90f + float.Lerp(-20f, 20f, XORShift128.Shared.NextFloat())) * float.Lerp(0.2f, 0.5f, XORShift128.Shared.NextFloat());
                if (float5.Y > this.Points[0][1].pos.Y - 0.1f)
                {
                    float5 = new Vector2(float5.X, float5.Y - 0.2f);
                }
                float num8 = float.Lerp(0.8f, 2f, XORShift128.Shared.NextFloat());
                if (XORShift128.Shared.NextFloat() < 0.5f)
                {
                    float5 += Custom.DegToFloat2(float.Lerp(120f, 170f, XORShift128.Shared.NextFloat())) * float.Lerp(0.1f, 0.3f, XORShift128.Shared.NextFloat());
                ...
                else
                {
                    ...
                    list.Add(new(float5 + Custom.DegToFloat2(float.Lerp(95f, 170f, XORShift128.Shared.NextFloat())) * float.Lerp(0.1f, 0.2f, XORShift128.Shared.NextFloat()), num8));
                }
                DefineBranch(list);
            }
            if (XORShift128.Shared.NextFloat() > 0.25f || !flag || elite)
            {
                ...
                list.Add(new(new Vector2(0f, 0f), 1f));
                float num10 = float.Lerp(95f, 135f, XORShift128.Shared.NextFloat());
                float num11 = float.Lerp(0.25f, 0.4f, XORShift128.Shared.NextFloat()) * num5;
                list.Add(new(Custom.DegToFloat2(num10) * num11, (flag2 ? 0.8f : float.Lerp(1f, num9, 0.3f)) * num5));
                list.Add(new(Custom.DegToFloat2(num10 + float.Lerp(5f, 35f, XORShift128.Shared.NextFloat())) * float.Max(num11 + 0.1f, float.Lerp(0.3f, 0.6f, XORShift128.Shared.NextFloat())), flag2 ? 0.8f : float.Lerp(1f, num9, 0.6f)));
                list.Add(new(Vector2.Normalize(list[list.Count - 1].pos) * (list[list.Count - 1].pos.Length() + float.Lerp(0.15f, 0.25f, XORShift128.Shared.NextFloat()) * num5), num9 * num6));
                DefineBranch(list);
            }
        }
    ...
    public struct IndividualVariations
    {
        ...
        public IndividualVariations(Personality personality, bool isElite = false)
        {
            ...
            EyeSize = float.Pow(float.Max(0f, float.Lerp(XORShift128.Shared.NextFloat(), float.Pow(HeadSize, 0.5f), XORShift128.Shared.NextFloat() * 0.4f)), float.Lerp(0.95f, 0.55f, personality.Sympathy));
            NarrowEyes = ((XORShift128.Shared.NextFloat() < float.Lerp(0.3f, 0.7f, personality.Sympathy)) ? 0f : float.Pow(XORShift128.Shared.NextFloat(), float.Lerp(0.5f, 1.5f, personality.Sympathy)));
            if (isElite)
            {
                NarrowEyes = 1f;
            }
            EyesAngle = float.Pow(XORShift128.Shared.NextFloat(), float.Lerp(2.5f, 0.5f, float.Pow(personality.Energy, 0.03f)));
            Fatness = float.Lerp(XORShift128.Shared.NextFloat(), personality.Dominance, XORShift128.Shared.NextFloat() * 0.2f);
            if (personality.Energy < 0.5f)
            {
                Fatness = float.Lerp(Fatness, 1f, XORShift128.Shared.NextFloat() * Custom.InverseLerp(0.5f, 0f, personality.Energy));
            }
            else
            {
                Fatness = float.Lerp(Fatness, 0f, XORShift128.Shared.NextFloat() * Custom.InverseLerp(0.5f, 1f, personality.Energy));
            }
            NarrowWaist = float.Lerp(float.Lerp(XORShift128.Shared.NextFloat(), 1f - Fatness, XORShift128.Shared.NextFloat()), 1f - personality.Energy, XORShift128.Shared.NextFloat());
            NeckThickness = float.Lerp(float.Pow(XORShift128.Shared.NextFloat(), 1.5f - personality.Aggression), 1f - Fatness, XORShift128.Shared.NextFloat() * 0.5f);
            PupilSize = 0f;
            DeepPupils = false;
            ColoredPupils = 0;
            if (XORShift128.Shared.NextFloat() < 0.65f && EyeSize > 0.4f && NarrowEyes < 0.3f)
            {
                ...
                {
                    PupilSize = float.Lerp(0.4f, 0.8f, float.Pow(XORShift128.Shared.NextFloat(), 0.5f));
            ...
            ArmThickness = float.Lerp(XORShift128.Shared.NextFloat(), float.Lerp(personality.Dominance, Fatness, 0.5f), XORShift128.Shared.NextFloat());
            ColoredEartlerTips = (isElite || XORShift128.Shared.NextFloat() < 1f / float.Lerp(1.2f, 10f, GeneralMelanin));
    ...
    public struct ScavColors
    {
        ...
        public ScavColors(Personality Personality, IndividualVariations Variations, bool Elite)
        {
            ...
            float num2 = num + float.Lerp(-1f, 1f, XORShift128.Shared.NextFloat()) * 0.3f * float.Pow(XORShift128.Shared.NextFloat(), 2f);
            if (num2 > 1f)
            {
                num2 -= 1f;
            }
            ...
            BodyColor = new HSLColor(num, float.Lerp(0.05f, 1f, float.Pow(XORShift128.Shared.NextFloat(), 0.85f)), float.Lerp(0.05f, 0.8f, XORShift128.Shared.NextFloat()));
            BodyColor.S = BodyColor.S * (1f - Variations.GeneralMelanin);
            BodyColor.L = float.Lerp(BodyColor.L, 0.5f + 0.5f * float.Pow(XORShift128.Shared.NextFloat(), 0.8f), 1f - Variations.GeneralMelanin);
            BodyColorBlack = Custom.LerpMap((BodyColor.RGB().R + BodyColor.RGB().G + BodyColor.RGB().B) / 3f, 0.04f, 0.8f, 0.3f, 0.95f, 0.5f);
            BodyColorBlack = float.Lerp(BodyColorBlack, float.Lerp(0.5f, 1f, XORShift128.Shared.NextFloat()), XORShift128.Shared.NextFloat() * XORShift128.Shared.NextFloat() * XORShift128.Shared.NextFloat());
            ...
            {
                num3 = num2 + float.Lerp(-1f, 1f, XORShift128.Shared.NextFloat()) * 0.1f * float.Pow(XORShift128.Shared.NextFloat(), 1.5f);
                num3 = float.Lerp(num3, 0.15f, XORShift128.Shared.NextFloat());
                if (num3 > 1f)
                {
                    num3 -= 1f;
                }
            ...
            {
                num3 = ((XORShift128.Shared.NextFloat() < 0.5f) ? Custom.Decimal(num + 0.5f) : Custom.Decimal(num2 + 0.5f)) + float.Lerp(-1f, 1f, XORShift128.Shared.NextFloat()) * 0.25f * float.Pow(XORShift128.Shared.NextFloat(), 2f);
                if (XORShift128.Shared.NextFloat() < float.Lerp(0.8f, 0.2f, Personality.Energy))
                {
                    num3 = float.Lerp(num3, 0.15f, XORShift128.Shared.NextFloat());
                }
                if (num3 > 1f)
            ...
            HeadColor.L = float.Lerp(HeadColor.L, 0.5f + 0.5f * float.Pow(XORShift128.Shared.NextFloat(), 0.8f), 1f - Variations.GeneralMelanin);
            ...
            HeadColorBlack = float.Lerp(HeadColorBlack, float.Lerp(0.8f, 1f, XORShift128.Shared.NextFloat()), XORShift128.Shared.NextFloat() * XORShift128.Shared.NextFloat() * XORShift128.Shared.NextFloat());
            ...
            DecorationColor.L = DecorationColor.L * float.Lerp(Variations.GeneralMelanin, XORShift128.Shared.NextFloat(), 0.5f);
            ...
            {
                HeadColor.L = float.Lerp(0.2f, 0.35f, XORShift128.Shared.NextFloat());
                HeadColorBlack *= float.Lerp(1f, 0.8f, XORShift128.Shared.NextFloat());
                BellyColor.H = float.Lerp(BellyColor.H, HeadColor.H, float.Pow(XORShift128.Shared.NextFloat(), 0.5f));
            }
    ...
    public struct ScavSkills
    {
		...
		private void SetUpCombatSkills(int seed, Personality personality, bool elite)
        {
            ...
            DodgeSkill = Custom.PushFromHalf(float.Lerp((XORShift128.Shared.NextFloat() < 0.5f) ? personality.Nervous : personality.Aggression, XORShift128.Shared.NextFloat(), XORShift128.Shared.NextFloat()), 1f + XORShift128.Shared.NextFloat());
            MidRangeSkill = Custom.PushFromHalf(float.Lerp((XORShift128.Shared.NextFloat() < 0.5f) ? personality.Energy : personality.Aggression, XORShift128.Shared.NextFloat(), XORShift128.Shared.NextFloat()), 1f + XORShift128.Shared.NextFloat());
            MeleeSkill = Custom.PushFromHalf(XORShift128.Shared.NextFloat(), 1f + XORShift128.Shared.NextFloat());
            BlockingSkill = Custom.PushFromHalf(Custom.InverseLerp(0.35f, 1f, float.Lerp((XORShift128.Shared.NextFloat() < 0.5f) ? personality.Bravery : personality.Energy, XORShift128.Shared.NextFloat(), XORShift128.Shared.NextFloat())), 1f + XORShift128.Shared.NextFloat());
            ReactionSkill = Custom.PushFromHalf(float.Lerp(personality.Energy, XORShift128.Shared.NextFloat(), XORShift128.Shared.NextFloat()), 1f + XORShift128.Shared.NextFloat());
