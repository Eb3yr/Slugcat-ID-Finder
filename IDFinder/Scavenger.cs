...
namespace IDFinder
{
    public class Scavenger
    {
        ...
        public Scavenger(int ID, bool isElite = false)
        {
            ...
            float num2 = Custom.Lerp(0.5f, 1.5f, float.Pow(XORShift128.NextFloat(), 1.5f - Personality.Aggression));
            num2 = Custom.Lerp(num2, num2 * Custom.LerpMap(teeth.GetLength(0), 4f, 8f, 1f, 0.5f), 0.3f);
            float num3 = Custom.Lerp(num2 + 0.2f, Custom.Lerp(0.7f, 1.2f, XORShift128.NextFloat()), XORShift128.NextFloat());
		...
		internal static (IndividualVariations? variations, ScavColors? color, BackTuftsAndRidges? back) GetGraphicsRNGParam(int ID, InstanceXORShift128 XORShift128, bool Elite = false, bool genVariations = false, bool genColors = false, bool genBack = false, Personality? inPersonality = null)
		{
			// Only graphics
			// Excludes eartlers as I don't (yet?) have any way to get meaningful information out of them. Maybe in the future.
			Personality personality;
    ...
    public struct Eartlers
    {
		...
		private void GenerateSegmentsRNGParam(bool elite, InstanceXORShift128 XORShift128)
		{
			float num = elite ? 1.75f : 1f;
			Vector2 @float = new(elite ? 45f : 15f, elite ? 90f : 45f);
			float rhs = elite ? 1.5f : 1f;
    ...
    public struct ScavColors
    {
		...
		internal ScavColors(Personality Personality, IndividualVariations Variations, bool Elite, InstanceXORShift128 XORShift128)
		{
			HSLColor BellyColor, BodyColor, DecorationColor, EyeColor, HeadColor;
			float num = XORShift128.NextFloat() * 0.1f;
			if (XORShift128.NextFloat() < 0.025f)
    ...
    public struct ScavSkills
    {
		...
		internal ScavSkills(int seed, Personality personality, InstanceXORShift128 XORShift128, bool elite = false)
		{
		...
		private void SetUpCombatSkillsRNGParam(int seed, Personality personality, bool elite, InstanceXORShift128 XORShift128)
		{
			uint x = XORShift128.x, y = XORShift128.y, z = XORShift128.z, w = XORShift128.w;    // preserving state.
			XORShift128.InitSeed(seed);
			DodgeSkill = Custom.PushFromHalf(Custom.Lerp((XORShift128.NextFloat() < 0.5f) ? personality.Nervous : personality.Aggression, XORShift128.NextFloat(), XORShift128.NextFloat()), 1f + XORShift128.NextFloat());
